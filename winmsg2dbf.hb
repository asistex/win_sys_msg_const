...
PROCEDURE Main()
   ...
   LOCAL oFile := TFileRead():new( "win_msg_codes.txt" )
   LOCAL n, h, t, i := 0, a, l, c

   IF hb_FileExists( "win_sys_msg.dbf" )
      Ferase( "win_sys_msg.dbf" )
   ENDIF
   Ferase( "win_sys_msg.cdx" )

   DBcreate( "win_sys_msg.dbf", { {"NUM_MSG","N",10,0},;
                                  {"DEF_MSG","C",65,0},;
                                  {"HEX_MSG","C",10,0},;
                                  {"TXT_MSG","C",650,0};
                                } )

   dbUseArea( .T., "DBFCDX", "win_sys_msg", "win_msg", .F., .F. )
